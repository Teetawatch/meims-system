<x-layouts.student :title="'Dashboard - ' . $student->first_name_th">
    ...
    <main class="flex-1 lg:ml-72 relative p-6 md:p-10 overflow-y-auto">
        ...
        <div class="relative rounded-[2rem] overflow-hidden p-8 md:p-12 mb-8 animate-fade-in-up"
            ...
            <div class="relative z-10 flex flex-col md:flex-row items-center gap-8">
                ...
                <div class="text-center md:text-left flex-1">
                    ...
                    <h2 class="text-text text-3xl md:text-4xl lg:text-5xl font-bold leading-tight mb-4 tracking-tight">
                    ...
                    <div class="flex flex-wrap justify-center md:justify-start gap-2.5">
                        <span class="inline-flex items-center gap-2 px-4 py-2 glass-card rounded-xl text-text-secondary text-sm font-bold border border-white/60 shadow-sm">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5 mb-8">
            ...
            <div class="stat-card glass-card rounded-2xl p-6 border border-white/60 shadow-sm hover:shadow-lg hover:shadow-orange-100/50 transition-all duration-300 hover:-translate-y-1.5 cursor-pointer group animate-fade-in-up-1">
                ...
                <p class="text-text-muted text-xs font-semibold mb-1">ความประพฤติ</p>
                <h4 class="stat-value text-2xl md:text-3xl font-bold text-text transition-transform duration-300">{{ $student->total_conduct_score }}</h4>
            ...
            <div class="stat-card glass-card rounded-2xl p-6 border border-white/60 shadow-sm hover:shadow-lg hover:shadow-emerald-100/50 transition-all duration-300 hover:-translate-y-1.5 cursor-pointer group animate-fade-in-up-2">
                ...
                <p class="text-text-muted text-xs font-semibold mb-1">GPA ล่าสุด</p>
                <h4 class="stat-value text-2xl md:text-3xl font-bold text-text transition-transform duration-300">
            ...
            <div class="stat-card glass-card rounded-2xl p-6 border border-white/60 shadow-sm hover:shadow-lg hover:shadow-blue-100/50 transition-all duration-300 hover:-translate-y-1.5 cursor-pointer group animate-fade-in-up-3">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="w-8 h-8 rounded-lg bg-info-light flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <svg class="w-4 h-4 text-primary-light" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                        </svg>
                    </div>
                </div>
                <p class="text-text-muted text-xs font-semibold mb-1">พุทธศักราช</p>
                <h4 class="stat-value text-2xl md:text-3xl font-bold text-text transition-transform duration-300">{{ $student->fiscal_year }}</h4>
            ...
            <div class="stat-card glass-card rounded-2xl p-6 border border-white/60 shadow-sm hover:shadow-lg hover:shadow-violet-100/50 transition-all duration-300 hover:-translate-y-1.5 cursor-pointer group animate-fade-in-up-4">
                ...
                <p class="text-text-muted text-xs font-semibold mb-1">รุ่น / Batch</p>
                <h4 class="stat-value text-2xl md:text-3xl font-bold text-text transition-transform duration-300">{{ $student->batch }}</h4>
                <div class="mt-2 flex gap-1">
                ...
                <div class="glass-card-strong rounded-2xl border border-white/60 shadow-sm overflow-hidden h-full">
                    <div class="p-6 pb-4 flex justify-between items-center border-b border-slate-100/60">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-lg font-bold text-text">ตารางเรียนวันนี้</h3>
                        </div>
                        <a href="{{ route('student.timetable') }}"
                            class="inline-flex items-center gap-1.5 text-primary text-sm font-bold hover:text-blue-700 transition-colors cursor-pointer group/link">
                            ดูทั้งหมด
                    ...
                    <div class="p-4 space-y-2">
                        ...
                        @forelse($courses as $index => $subject)
                            <div class="flex items-center gap-5 p-4 rounded-xl hover:bg-white/60 transition-all duration-200 border border-transparent hover:border-slate-100/80 hover:shadow-sm group cursor-pointer">
                                <div class="w-14 h-14 rounded-xl flex flex-col items-center justify-center text-white font-bold group-hover:scale-105 transition-transform duration-200 shadow-sm shrink-0"
                                ...
                                <div class="flex-1 min-w-0">
                                    <h5 class="font-bold text-text text-sm truncate">{{ $subject->subject_name_th }}</h5>
                                    <p class="text-xs text-text-disabled font-medium mt-0.5 flex items-center gap-1.5">
                        ...
                        @empty
                            <div class="py-16 text-center">
                                ...
                                <p class="text-text-disabled font-medium text-sm">ไม่มีตารางเรียนในวันนี้</p>
                                <p class="text-text-disabled text-xs mt-1">พักผ่อนให้เต็มที่นะ</p>
                            </div>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="glass-card-strong rounded-2xl border border-white/60 shadow-sm h-full flex flex-col">
                    <div class="p-6 pb-4 border-b border-slate-100/60">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="text-lg font-bold text-text">ความประพฤติล่าสุด</h3>
                        </div>
                    </div>
                    <div class="p-5 flex-1 space-y-4 overflow-y-auto max-h-[380px]">
                            ...
                            <div class="flex items-start gap-3.5 p-3 rounded-xl hover:bg-white/50 transition-all duration-200 cursor-pointer">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex justify-between items-start gap-2">
                                        <h5 class="text-sm font-bold text-text-secondary truncate">{{ $score->description }}</h5>
                                        <span class="text-xs font-bold shrink-0 px-2 py-0.5 rounded-md {{ $score->score > 0 ? 'text-emerald-700 bg-emerald-50' : 'text-error bg-red-50' }}">
                                            {{ $score->score > 0 ? '+' : '' }}{{ $score->score }}
                                        </span>
                                    </div>
                                    <p class="text-[11px] text-text-disabled font-medium mt-1">
                        ...
                        @empty
                            <div class="h-full flex flex-col items-center justify-center py-12">
                                ...
                                <p class="text-text-disabled font-medium text-sm">ยังไม่มีประวัติความประพฤติ</p>
                            </div>
                        @endforelse
                    </div>
            ...
            <div class="glass-card-strong rounded-2xl border border-white/60 shadow-sm overflow-hidden">
                <div class="p-6 pb-4 flex justify-between items-center border-b border-slate-100/60">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-lg font-bold text-text">เอกสารเผยแพร่ล่าสุด</h3>
                    </div>
                ...
                <div class="p-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    @forelse($recentDocuments as $index => $doc)
                        <div class="group relative p-4 rounded-xl border border-slate-100/60 hover:border-indigo-200/60 hover:bg-white/60 hover:shadow-md hover:shadow-indigo-50/50 transition-all duration-300 cursor-pointer">
                            <div class="flex items-start gap-3.5">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h5 class="text-sm font-bold text-text-secondary truncate group-hover:text-indigo-600 transition-colors duration-200">
                                        {{ $doc->title }}</h5>
                                    <p class="text-[11px] text-text-disabled font-semibold mt-1">
                                        {{ $doc->file_size }}</p>
                                </div>
                            </div>
                            <a href="{{ asset('storage/' . $doc->file_path) }}" target="_blank"
                                class="absolute top-3 right-3 w-8 h-8 rounded-lg flex items-center justify-center text-text-disabled hover:text-indigo-600 hover:bg-indigo-50 transition-all duration-200 opacity-0 group-hover:opacity-100 cursor-pointer">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @empty
                        <div class="col-span-full py-12 text-center">
                            ...
                            <p class="text-text-disabled font-medium text-sm">ยังไม่มีเอกสารใหม่ในขณะนี้</p>
                        </div>
                    @endforelse
                </div>
            </div>
